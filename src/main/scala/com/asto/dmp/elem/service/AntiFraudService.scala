...
class AntiFraudService extends DataSource with Serializable {
  ...
  def run(): Unit = {
    ...
    try {
      FileUtils.deleteHdfsFile(Constants.OutputPath.ANTI_FRAUD_PARQUET)
      FileUtils.deleteHdfsFile(Constants.OutputPath.ANTI_FRAUD_TEXT)
      ...
      val result = FQZ1.leftOuterJoin(FQZ2).filter(t => t._2._2.isDefined). //(12663421529953153,((0.7950705636866545,false),Some((0.8342663644556104,false))))
      ...
      result.toDF("订单ID", "订单日期", "餐厅ID", "餐厅名称", "下单客户ID", "下单时间", "订单额", "刷单指标值1", "刷单指标值2", "刷单指标值3", "刷单指标值4", "刷单指标值5", "是否刷单").write.parquet(Constants.OutputPath.ANTI_FRAUD_PARQUET)
      result.map(_.productIterator.mkString(",")).coalesce(1).saveAsTextFile(Constants.OutputPath.ANTI_FRAUD_TEXT)
    } catch {
      case t: Throwable =>
