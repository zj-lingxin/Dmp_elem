...
class AntiFraudService extends DataSource with Serializable {
  ...
  def run(): Unit = {
    ...
    try {
      ...
      fakedInfo.persist()
      FileUtils.deleteHdfsFiles(Constants.OutputPath.ANTI_FRAUD_FAKED_INFO_PARQUET, Constants.OutputPath.ANTI_FRAUD_FAKED_INFO_TEXT)
      import sqlContext.implicits._
      fakedInfo.toDF("订单ID", "订单日期", "餐厅ID", "餐厅名称", "下单客户ID", "下单时间", "订单额", "刷单指标值1", "刷单指标值2", "刷单指标值3", "刷单指标值4", "刷单指标值5", "是否刷单").write.parquet(Constants.OutputPath.ANTI_FRAUD_FAKED_INFO_PARQUET)
      ...
      lastSixMonthsFakedRate.toDF("餐厅ID", "餐厅名称", "近6个月刷单金额", "近6个月总营业额", "刷单率").write.parquet(Constants.OutputPath.ANTI_FRAUD_FAKED_RATE_PARQUET)
