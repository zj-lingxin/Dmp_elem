...
class AntiFraudService extends DataSource with Serializable {
  ...
  def run(): Unit = {
    try {
      ...
      val fakedInfoRDD = FQZ1.leftOuterJoin(FQZ2).filter(t => t._2._2.isDefined) //(12663421529953153,((0.7950705636866545,false),Some((0.8342663644556104,false))))
      ...
      fakedInfoRDD.persist()
      FileUtils.saveAsTextFile(fakedInfoRDD, Constants.OutputPath.ANTI_FRAUD_FAKED_INFO_TEXT)

      //输出2：餐厅ID,餐厅名称,近6个月刷单金额,近6个月总营业额,刷单率
      val lastSixMonthsList = BizUtils.getLastMonths(6, Constants.App.YEAR_MONTH_FORMAT)
      val result2NeedsData = fakedInfoRDD.map(t => (DateUtils.cutYearMonth(t._2), t._3, t._4, t._7, t._13))
      ...
      FileUtils.saveAsTextFile(lastSixMonthsFakedRate, Constants.OutputPath.ANTI_FRAUD_FAKED_RATE_TEXT)
      fakedInfoRDD.unpersist()
    } catch {
      case t: Throwable =>
